...
const checkDogs = (dogsJulia, dogsKate) => {
  const juliaWithNoCats = dogsJulia.slice(1, -2);
  const combinedDogs = juliaWithNoCats.concat(dogsKate);

  combinedDogs.forEach((age, idx) => {
    console.log(
      `Dog number ${idx + 1} is ${age >= 3 ? `an adult, and is ${age} years old` : 'is still a puppy 🐶'}`,
    );
  });
};
...
const setSwimming = new Set(
  breeds
    .filter(breed => breed.activities.includes('swimming'))
    .flatMap(breed => breed.activities),
);
setSwimming.delete('swimming');
const swimmingAdjacent = [...setSwimming];

const doAllBreedsWeight10kgOnAverage = breeds.every(
...
//2.
console.log(
  "Is Sarah's dog eating too much? ",
